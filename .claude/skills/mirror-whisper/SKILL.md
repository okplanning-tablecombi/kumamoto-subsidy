...
- 全編が同一文の繰り返し(「お疲れ様です」等)になったら、それは静かな窓の
  幻覚が窓間文脈で全体に伝搬したもの。スクリプトは既定で n_max_text_ctx=0 を
  設定して防いでいる(whisper.cpp の no_context=True はこれを防がない。
  同一呼び出し内では prompt_past1 が無条件に引き継がれるため)。孤立した
  1窓だけの唐突な短文(「おやすみなさい」等)は静音区間の幻覚なので除外する。
